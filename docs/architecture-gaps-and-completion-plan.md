...
> 基于项目实际代码审计（2026-04-18 更新）

## 文档信息

- **创建日期**: 2026-04-15
- **最后更新**: 2026-04-18（代码审计更新）
- **参考来源**:
  - 当前项目源码全面审计 (`src/` 逐文件审查)
  - [AI SDK v6 官方文档](https://ai-sdk.dev/docs)
  - AI SDK v6.0.149 实际 API
  - 新增设计文档：`docs/permission-control-design.md`、`docs/recovery-retry-and-fallback-plan.md`
...
### 1.2 当前已有能力（2026-04-18 审计确认）

| 能力 | 状态 | 说明 | 源码位置 |
|---|---|---|---|
| **文件级记忆系统** | ✅ | 完整实现：文件存储 + MEMORY.md + 四类型分类法 + 智能召回 + 老化追踪 + LLM 提取 + SQLite 索引 + **CRUD 操作 + 冲突检测** | `src/lib/memory/` (9 文件) |
...
| **MCP 注册表** | ✅ | SSE/HTTP/stdio 传输，工具过滤，多服务器管理 | `src/lib/mcp/` |
| **MCP Pipeline 连接** | ✅ | MCP 工具已接入 chat route allTools，动态加载 + 连接管理 | `src/app/api/chat/route.ts:188-198` |
| **needsApproval 审批流（基础）** | ✅ | Bash 黑白名单 + ask_user_question + 前端审批 UI + 响应流程 | `src/lib/tools/bash.ts` + `Chat.tsx` |
...
| **记忆 CRUD 操作** | ✅ | create/update/delete/invalidate 四种操作类型 + 冲突检测 | `src/lib/memory/store.ts` + `extractor.ts` |

### 1.3 完全缺失的关键能力

| Claude Code 能力 | 当前项目 | 影响 | 备注 |
|---|---|---|---|
| API 自适应重试 | ❌ | 高（稳定性） | **已有完整设计文档** `docs/recovery-retry-and-fallback-plan.md` |
| 多层恢复路径（413 紧急压缩链） | ❌ | 高（413 无自动恢复） | **已有完整设计文档** `docs/recovery-retry-and-fallback-plan.md` |
| 失败触发模型回退 | ❌ | 高（连续故障无自动降级） | **已有完整设计文档** `docs/recovery-retry-and-fallback-plan.md` |
| 文件路径验证 | ❌ | 中（文件工具无限制） | **已有设计文档** `docs/permission-control-design.md` |
| 向量嵌入 RAG（基于 embed/re-rank） | ❌ | 中（记忆召回仅 keyword） | SDK 支持，待实现 |
| OTel 标准可观测性 | ❌ | 中（自定义遥测已存在） | 可升级为 OTel 标准 |
| QueryEngine 对话管理类 | ❌ | 中（工厂函数替代） | 架构优化项 |
| 工具级 Hooks 系统 | ❌ | 低（可手动实现） | SDK 无原生支持 |
| 分层 AbortController 树 | ❌ | 低（基础已够用） | 架构优化项 |
| 多提供商 SDK 集成 | ❌ | 高（仅 DashScope） | SDK 原生支持，待配置 |

---

## 2. 差距分类总览

根据项目实际代码审计结果更新（2026-04-18）：

| 分类 | 判定标准 | 项目数 |
|---|---|---|
| **✅ 已具备** | 当前项目已实现 | 25 |
| **🟡 可补齐** | SDK 有原生能力或可通过中间件/prepareStep 实现，**已有设计文档或部分实现** | 8 |
...
|---|---|---|---|---|---|---|
| **P0** | 文件级记忆系统 | 🟢 已具备 | 极高（跨对话） | - | ❌ 缺失 | ✅ **已完整实现 + CRUD** |
| **P0** | 权限系统（基础审批） | 🟢 已具备 | 高（安全） | - | ✅ **已实现** | ✅ Bash + ask_user_question + 前端 UI |
| **P1** | 文件路径验证 | 🟡 | 中（文件工具无限制） | 小 | ✅ **已有设计文档** | ❌ 待实现 |
| **P0** | API 自适应重试 | 🟡 | 高（稳定性） | 中 | ✅ **已有设计文档** | ⚠️ 设计完成，代码待实现 |
| **P0** | 多层恢复路径 | 🟡 | 高（413 恢复） | 中 | ✅ **已有设计文档** | ⚠️ 设计完成，代码待实现 |
| **P1** | 文件路径验证 | 🟡 | 中 | 小 | ✅ **已有设计文档** | ❌ 待实现 read/edit/write needsApproval |
| **P1** | 失败触发模型回退 | 🟡 | 高 | 小 | ✅ **已有设计文档** | ⚠️ 设计完成，代码待实现 |
| **P1** | SDK `needsApproval` 审批流 | 🟢 已具备 | 高 | - | ✅ **Phase 1 部分** | ✅ Bash + ask_user_question + 前端 UI 已实现 |
...
| **P1** | MCP Pipeline 连接 | 🟢 已具备 | 高 | - | ⚠️ 原标记缺失 | ✅ **已实现** `route.ts:188-198` |
| **P1** | SDK `needsApproval` 审批流 | 🟡 | 高 | 中 | ✅ **已有设计文档** | ⚠️ 设计完成，代码待实现 |
...
| **P0** | 记忆 CRUD 操作 | 🟢 已具备 | 中 | - | ❌ | ✅ **store.ts 已实现** |
| **P0** | 记忆冲突检测 | 🟢 已具备 | 中 | - | ❌ | ✅ **extractor.ts 已实现** |

**上次文档 vs 本次审计：新增 3 项已实现能力（MCP 连接、记忆 CRUD、冲突检测）**
...
**上次文档状态**: ❌ 无正式权限系统
**实际项目状态**: ⚠️ **基础审批已实现**，路径验证待完善

**已实现**:
- ✅ Bash 工具 `needsApproval` — 黑名单 + 白名单 + 需审批判定（`src/lib/tools/bash.ts:167-181`）
- ✅ ask_user_question 工具 `needsApproval: async () => true`（`src/lib/tools/ask-user-question.ts:41`）
- ✅ 前端审批 UI — `ApprovalPanel` + `UserQuestionPanel`（`src/components/ai-elements/`）
- ✅ 审批响应流程 — `addToolApprovalResponse` + `approval-requested` 状态检测（`src/components/Chat.tsx:215-325`）

**待完善**:
- ❌ 文件路径验证 — read/edit/write 工具无 `needsApproval`，可操作任意路径

**不再需要（单用户场景）**:
- RBAC 用户角色系统
- 权限模式切换（plan/auto/bypass）
- AI 命令安全分类器
- 规则持久化（可选）

**影响**：中。当前基础审批已可用，文件路径验证是主要待完善项。
...
**上次文档状态**: ❌ 无 API 级重试
**实际项目状态**: ⚠️ **已有完整设计文档** `docs/recovery-retry-and-fallback-plan.md`（2026-04-15），代码待实现

**设计文档已规划（待实现）**:
- ✅ `retryMiddleware` — 指数退避重试（max 3 次，500ms base delay + jitter）
- ✅ 可重试错误判定：429/5xx/timeout/网络错误
- ✅ 不可重试错误：413/400/403（交由恢复链处理）
- ✅ 流式调用重试（`wrapStream` 捕获连接前错误）
- ✅ 中间件链顺序：retry → telemetry → costTracking
- ✅ `onConsecutiveFailures` 回调（触发模型回退）

**当前**: 无任何 LLM API 级重试。DashScope 调用失败直接传播为错误。

**影响**：高。网络抖动或 API 限流会直接导致失败。**设计文档已完备，预计 2 天实现。**
...
**设计文档已规划的恢复链（`src/lib/compaction/recovery.ts`）**:
```typescript
recoverFromContextError(messages, conversationId) →
  1. compactViaAPI (LLM 压缩) → 成功 → 重试
  2. trySessionMemoryCompact (DB 摘要) → 成功 → 重试
  3. microCompactMessages (清除旧工具结果) → 省出 >5K → 重试
  4. truncateForPTLRetry (丢弃 20% 最旧) → 成功 → 重试
  5. tryPtlDegradation (硬截断兜底) → 一定成功 → 重试
```

**关键设计决策（基于 SDK 类型审查）**:
- ✅ 确认 `StepResult` 无 `error` 字段，API 异常不产生 StepResult
- ✅ 恢复必须在 **route 层 catch** 处理，无法在 `prepareStep` 中实现
- ✅ 使用 `UIMessage[]` 类型（与现有压缩函数一致）
- ✅ MAX_RECOVERY_ATTEMPTS = 3 硬限制防止无限循环

**影响**：高。上下文溢出时无自动恢复。**设计文档已完备，预计 4 天实现。**
...
**实际项目状态**: ⚠️ 确认仅支持意图/成本/复杂度切换，**设计文档已规划失败触发机制**
...
**设计文档已规划（`docs/recovery-retry-and-fallback-plan.md` Phase 3）**:
- ✅ `retryMiddleware` 增加 `onConsecutiveFailures` 回调
- ✅ 连续 3 次失败时设置 `sessionState.needsModelFallback = true`
- ✅ `prepareStep` 读取标记，返回 `{ model: wrappedFallbackModel }`
- ✅ `ModelSwapper.getFallbackModel()` 返回更低成本模型
- ✅ 成功后重置 `needsModelFallback` 标记

**影响**：高。连续故障无自动降级。**设计文档已完备，预计 1 天实现。**
...
**上次文档状态**: ⚠️ 基础集成
**实际项目状态**: ✅ 注册表实现，⚠️ WebSocket/OAuth 非必需

**有的**: SSE/HTTP/stdio 传输支持，工具过滤，多服务器管理
**无**: WebSocket, OAuth（企业场景下非必需）
...
**上次文档状态**: ⚠️ 有注册表但未连接
**实际项目状态**: ✅ **已实现** `src/app/api/chat/route.ts:188-198`

**实现代码**:
```typescript
// route.ts 第 188-198 行
if (mcpRegistry) {
...
**连接流程**:
1. POST handler 调用 `getMcpServerConfigs()` 获取配置
2. 创建 `createMcpRegistry(mcpConfigs)` 注册表实例
3. `await mcpRegistry.connectAll()` 连接所有 MCP 服务器
4. MCP 工具添加到 `allTools`，工具名前缀 `mcp_`
5. Agent 创建时传入 `mcpRegistry`
6. `onFinish` 中 `await mcpRegistry.disconnectAll()` 清理连接

**状态**: ✅ MCP 工具已完整接入 Pipeline，动态加载 + 连接管理已实现。
...
> **注**: 本节描述的设计方案已完整记录在以下文档中：
> - `docs/recovery-retry-and-fallback-plan.md` — API 重试 + 413 恢复 + 模型回退
> - `docs/permission-control-design.md` — 权限控制体系

### 9.1 🟡 可补齐（AI SDK 原生/中间件支持）

#### 9.1.1 API 自适应重试（LM Middleware）

> **状态**: 设计完成，代码待实现。详见 `docs/recovery-retry-and-fallback-plan.md` Phase 1
...
#### 9.1.2 多层恢复路径（route 层 catch）

> **状态**: 设计完成，代码待实现。详见 `docs/recovery-retry-and-fallback-plan.md` Phase 2
> **关键修正**: 原计划在 `prepareStep` 中检测错误，但 SDK 类型审查发现 `StepResult` 无 `error` 字段，API 异常不产生 StepResult，必须在 route 层 catch 处理
...
> **状态**: 设计完成，代码待实现。详见 `docs/recovery-retry-and-fallback-plan.md` Phase 3
...
> **状态**: 设计完成，代码待实现。详见 `docs/permission-control-design.md`
> **预计工作量**: 4-5 周（三阶段递进实施）
...
> **状态**: ✅ **已实现** `src/app/api/chat/route.ts:188-198`

MCP 工具已正确接入 chat route，无需额外实现。
...
### 10.1 基于实际代码审计的更新（2026-04-18）

| 需求 | 优先级 | 当前状态 | SDK 实现方式 | 工作量 | 设计文档 |
|---|---|---|---|---|---|
| **API 自适应重试** | P0 | ⚠️ 设计完成 | SDK LM Middleware | 2 天 | `docs/recovery-retry-and-fallback-plan.md` |
| **多层恢复路径（413）** | P0 | ⚠️ 设计完成 | route 层 catch + 降级链 | 4 天 | `docs/recovery-retry-and-fallback-plan.md` |
| **文件路径验证** | P1 | ❌ 未实现 | SDK `needsApproval` | 3 天 | `docs/permission-control-design.md` |
| **失败触发模型回退** | P1 | ⚠️ 设计完成 | SDK `prepareStep` + retryMiddleware 回调 | 1 天 | `docs/recovery-retry-and-fallback-plan.md` |
| **MCP 工具接入 Pipeline** | P1 | ✅ 已实现 | 导出 MCP tools 到 agent | - | 无需 |
| **向量 RAG** | P1 | ❌ keyword 替代 | SDK Middleware + embed | 2 周 | 待创建 |
| **多提供商配置** | P1 | ⚠️ SDK 支持但未配置 | SDK 原生 | 1 周 | 待创建 |
| **OTel 标准化** | P2 | ⚠️ 自定义遥测 | SDK Telemetry API | 1 周 | 待创建 |

### 10.2 不再需要实施的项（已具备）

| 需求 | 实际实现 | 源码位置 |
|---|---|---|
| 文件级记忆系统 | ✅ 完整实现 + CRUD + 冲突检测 | `src/lib/memory/` (9 文件) |
| MCP Pipeline 连接 | ✅ 已接入 allTools + 连接管理 | `src/app/api/chat/route.ts:188-198` |
| **SDK needsApproval 审批流（基础）** | ✅ Bash/ask_user_question + 前端 UI | `src/lib/tools/bash.ts:167-181` + `Chat.tsx:215-325` |
...
| 记忆 CRUD 操作 | ✅ 四种操作类型 + SQLite | `src/lib/memory/store.ts` |
| 记忆冲突检测 | ✅ 现有记忆扫描 + 上下文 | `src/lib/memory/extractor.ts` |

### 10.3 时间线总览（更新后）

```
第 1-2 天:   Phase 1 — retryMiddleware 编写 + 接入 route.ts（设计文档已完备）
第 3-6 天:   Phase 2 — recovery.ts 编写 + route.ts 接入（设计文档已完备）
第 7 天:     Phase 3 — ModelSwapper.getFallbackModel + retryMiddleware 回调（设计文档已完备）
第 8-10 天:  文件路径验证 — read/edit/write 工具 needsApproval
第 11-24 天: 向量 RAG（embed + rerank）
第 25-31 天: 多提供商配置
第 32-38 天: OTel 标准化
```

**总计**：约 1.5 个月可补齐所有剩余差距项。

**关键变化**：
1. MCP Pipeline 连接已实现，无需额外时间
2. needsApproval 审批流已实现，仅需完善文件路径验证（约 3 天）
3. 不再需要 RBAC 和权限模式切换（单用户场景）
4. API 重试/恢复/回退的设计文档已完备，实现时间缩短
...
经过 2026-04-18 全面代码审计，项目已实现 **25 项核心能力**，包括：
- MCP Pipeline 连接（原标记为缺失）
- needsApproval 审批流（Bash + ask_user_question + 前端 UI）

**剩余高价值差距项**：
- API 重试 + 413 恢复 + 模型回退（设计文档完备，预计 7 天实现）
- 文件路径验证（read/edit/write 工具，预计 3 天实现）
- 向量 RAG + 多提供商配置 + OTel（按需实施）
...
| 文档 | 创建日期 | 状态 | 描述 |
|---|---|---|---|
| `docs/recovery-retry-and-fallback-plan.md` | 2026-04-15 | ✅ 设计完成 | API 重试中间件 + 413 恢复链 + 模型回退 |
| `docs/permission-control-design.md` | 2026-04-16 | ✅ **简化更新** | 基础审批已实现，路径验证待完善，移除 RBAC |
| `docs/connector-gateway-design-v2.md` | - | 待审查 | 连接器网关设计 |
