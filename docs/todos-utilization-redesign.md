...
> - ✅ Phase A：规划触发——开工提示 + 5 步兜底 + 修死空清单提醒路径；**2026-08-15 重构弃用关键词判复杂度，改由大模型判断 + harness 提醒**（g01 1/7 → 稳定 2/2）
...
| 级 | 行为 | 状态（2026-08-15） |
|----|------|----|
| 1 | 建——多步请求开头建清单 | ✅ Phase A 规划触发（LLM 判断 + harness 提醒） |
...
## 3. Phase A：规划触发（LLM 判断 + harness 提醒，rung 1-2）

### 3.1 设计（2026-08-15 重构：弃用所有规则判复杂度）

> 初版曾用正则/关键词做「多步判定」（`looksMultiStep`/`executionIntent`，执行型硬注入 +
> 反复提醒），后又加了字数门限判「超短请求」。**全部弃用**——"顺便/还有"等话术判不出、
> "请给我生成一个贪吃的游戏"很短却是复杂任务，**任何分类都该交给模型，代码只做确定性的
> 机械活（何时提醒）**。
>
> **复杂度 = 心智复杂度，不是实现步骤数**（2026-08-15 用户明确）。todo 的作用是**拆分
> 复杂任务**——该不该规划看问题需不需要拆开思考（涉及多方面 / 有取舍 / 结果不确定 /
> 要探索迭代），而不是"要做几步"。一个 10 步的机械活不需要 todo，一个 2 步的重大决策
> 可能非常需要。提示词据此改为按"心智复杂度"引导，并明确"即使实际步骤不多"。

`packages/core/src/modules/agent-control/plan-prompt.ts`：

- `buildPlanPrompt()`：开工前轻问一句——"如果这个请求有较高复杂度（需要把较大问题拆成
  几个子问题来思考和完成：涉及多方面 / 有取舍 / 结果不确定 / 要探索迭代），即使实际步骤
  不多，也请先用 todo_write 拆成任务清单；如果简单直接，直接回答"。**判断交给模型**。
...
不再有执行型/内容型的硬软分叉、不再按"开始干活"反复念叨、不再有字数门限、也不再按
"步骤/交付物"教模型——判断完全交回模型，harness 职责收敛为"提醒"。

### 3.2 验证（同一 harness，deepseek-v4-pro）

| 用例 | Phase 0 基线 | 重构后 |
|------|------|------|
| g01 行程 | 1/7 | **2/2**（step0 建 5 项，稳定） |
| c02 重构 | 1/3 | 1/2（模型判断有波动，建单那次推进到完成） |
| g02 整理 | 0/1 | 0/2（内容型，模型常判"可直接总结"——正是交给模型判断的应有表现） |
| 单步误建率 | 0% | 0% |

### 3.3 天花板（诚实记录）

无论谁做判断（规则或模型），**提醒终究是"话"，模型可以无视**（c02 曾多次提醒仍直接干完）。
c02/g02 的 ~50% 波动是"交给 LLM 判断"的固有方差。逼近 100% 需**工具级强制**（清单不存在时
拦截非 todo 工具执行），风险：可能死锁（模型不建单就卡住）、改变用户可见行为。见 §6。
