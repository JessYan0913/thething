...
### 3.1 设计（2026-08-15 重构：弃用关键词判复杂度）

> 初版曾用正则/关键词做「多步判定」（`looksMultiStep`/`executionIntent`，执行型硬注入 +
> 反复提醒）。**判断质量太粗**（"顺便""还有"等话术判不出，且不如模型懂复杂度的语义），
> 已重构为：**"是否多步 / 是否要规划"交给大模型判断，harness 只负责提醒**。

`packages/core/src/modules/agent-control/multi-step-detector.ts`：

- `buildPlanPrompt()`：开工前轻问一句——"如果这个请求需要多步/多项交付物/多次操作，
  先用 todo_write 建清单；如果单步/纯问答/输出即答案，直接回答"。**建不建由模型判断**。
- `buildEmptyTodoReminder()`：5 步兜底——"已执行多步但仍未建清单，如果确实需要多步
  先建清单；确认单步就忽略"。**不预判是否多步，由模型再判断一次**。
- `isTrivialRequest(text)`：唯一保留的确定性门槛——只跳过超短请求（<12 字，如"现在几点"），
  **纯粹为不烦人，不是复杂度判断**。
- `getLastUserText(messages)`：提取最后一条用户文本。

`pipeline.ts` 两处注入：

1. **开工前提示**（step 0，清单为空且非超短请求）：注入 `buildPlanPrompt()`——判断交给模型。
2. **5 步兜底**（清单为空 + 5 步无活动）：注入 `buildEmptyTodoReminder()`。**修死了原
   空清单不注入的死路径**。

不再有执行型/内容型的硬软分叉，也不再根据"开始干活"反复念叨——把判断完全交回模型，
harness 的职责收敛为"提醒"，避免用规则替模型拍板。
