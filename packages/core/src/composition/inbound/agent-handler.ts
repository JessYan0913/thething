  ...
  return messages.map(msg => {
    ...
    for (const part of msg.parts) {
      ...
      if (state === 'output-available' || state === 'output-error') {
        ...
        logger.warn('sanitizeMessages', `Skipping incomplete tool call: ${toolCallId} ${toolName}`)
      } else {
        sanitizedParts.push(part)
      }
    }
...
export interface AgentHandlerConfig {
  ...
  conversationResolver: { resolve(event: InboundEvent): Promise<string> }
...
export class AgentInboundHandler implements InboundEventHandler {
  ...
  private conversationResolver: { resolve(event: InboundEvent): Promise<string> }
  ...
  async handle(event: InboundEvent): Promise<InboundEventResult> {
    if (!claimInboundEvent(event.id)) {
      logger.warn('AgentInboundHandler', `Duplicate inbound event ignored: ${event.id}`)
      return { success: true }
    }

    const startTime = Date.now()
    logger.debug('AgentInboundHandler', `START HANDLING EVENT: ${event.id} connector=${event.connectorId} protocol=${event.protocol} channel=${event.channel.id} sender=${event.sender.id} text="${(event.message.text || '').slice(0, 50)}"`)

    try {
      const conversationId = await this.findOrCreateConversation(event)
      logger.debug('AgentInboundHandler', `Conversation ID: ${conversationId}`)
    ...
    while (round < MAX_ROUNDS) {
      ...
      for (const req of approvalRequests) {
        ...
        if (rule?.behavior === 'allow') {
          logger.debug('AgentInboundHandler', `Auto-approved by permissions.json: ${req.toolName}`)
          autoApprovalContent.push({ type: 'tool-approval-response', approvalId: req.approvalId, approved: true })
        } else if (rule?.behavior === 'deny') {
          logger.debug('AgentInboundHandler', `Denied by permissions.json: ${req.toolName}`)
          ...
          logger.debug('AgentInboundHandler', `Already approved by user in this session: ${req.toolName}`)
          autoApprovalContent.push({ type: 'tool-approval-response', approvalId: req.approvalId, approved: true })
        } else {
