...
export async function persistToolResult(
  ...
  try {
    ...
    logger.debug(
      'ToolResultStorage',
      `Persisted ${toolUseId} to ${filepath} (${formatSize(content.length)})`
    )
  } catch (error: unknown) {
    const errno = getErrnoCode(error)
    if (errno !== 'EEXIST') {
      logger.error('ToolResultStorage', 'Write error:', error)
      throw error
    }
    // EEXIST: 文件已存在，跳过写入
    logger.debug('ToolResultStorage', `File already exists: ${filepath}`)
  }
...
export async function cleanupSessionToolResults(
  ...
  try {
    ...
    logger.debug('ToolResultStorage', `Cleaned up session ${sessionId} tool results`)
  } catch (error: unknown) {
    const errno = getErrnoCode(error)
    if (errno !== 'ENOENT') {
      logger.warn('ToolResultStorage', 'Cleanup error:', error)
    }
    // ENOENT: 目录不存在，无需清理
  }
}
...
export async function cleanupOldToolResults(
  ...
  try {
    ...
    if (cleanedSessions > 0) {
      logger.debug(
        'ToolResultStorage',
        `Cleaned ${cleanedSessions} old sessions, ~${cleanedFiles} files`
      )
    }
    ...
    const errno = getErrnoCode(error)
    if (errno !== 'ENOENT') {
      logger.warn('ToolResultStorage', 'Cleanup old results error:', error)
    }
