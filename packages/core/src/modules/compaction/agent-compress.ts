 ...
 *
 * - 增量:若 summaryStore 已有摘要,作为【历史摘要】前置,模型在其基础上更新。
 * - 保真:restoreMissingPaths 确定性补回 tool-result 找回路径(不依赖模型行为)。
 * - 落库:提供 anchorMessageId 时写 summaryStore(Path A)。
 *
 * @returns success=false 表示摘要为空/质量不达标,调用方应回退(由闸门兜底,不 forceTruncate)
 */
export async function agentCompress(
  ...
  // 喂真实 ModelMessage(保留 toolCall / toolResult 结构、文件路径原文),不拍扁、不 slice
  const stripped = stripImagesFromMessages(olderMessages);
  const messagesForModel: ModelMessage[] = existing
    ? ([{ role: 'user', content: `【历史摘要】\n${existing}` } as ModelMessage, ...stripped])
    : stripped;

  const summary = await callWithFallback(messagesForModel, context.model, context.fallbackModels);
  if (!summary || !validateSummaryQuality(summary, olderMessages)) {
    logger.warn('AgentCompress', '摘要为空或质量不达标,放弃压缩(由闸门兜底)');
    return { success: false };
  }

  // 确定性补回:模型即便违反提示词丢掉路径,也在这里兜底补回
  ...
  if (restoredCount > 0) {
    logger.info('AgentCompress', `补回 ${restoredCount} 条丢失的 tool-result 找回路径`);
  }
