...
import { getUserTokenizerCacheDir } from "../../primitives/paths";
...
class TokenCounterImpl {
  ...
  registerTokenizer(modelName: string, configPath: string): void {
    ...
    logger.debug('TokenCounter', `注册 tokenizer: ${modelName} -> ${configPath}`);
  }
  ...
  setTokenizerDir(dir: string): void {
    this.userDir = dir;
    logger.debug('TokenCounter', `设置 tokenizer 目录: ${dir}`);
  }
  ...
  async getTokenizer(modelName: string): Promise<Tokenizer> {
    ...
    try {
      ...
      if (!this.warnedModels.has(modelName)) {
        logger.debug('TokenCounter', `已加载 ${modelName} tokenizer`);
  ...
  private async loadFromPath(modelName: string, filePath: string): Promise<Tokenizer> {
    ...
    try {
      ...
      logger.debug('TokenCounter', `从路径加载 ${modelName} tokenizer`);
      return tokenizer;
    } catch (error) {
  ...
  private async downloadIfNeeded(
    ...
    const downloadPromise = this.downloadTokenizerFiles(repoInfo, destDir)
      .catch((error) => {
        logger.error('TokenCounter', `下载失败: ${error instanceof Error ? error.message : error}`);
        throw error;
      });
  ...
  private async downloadTokenizerFiles(
    ...
    logger.debug('TokenCounter', `开始下载 ${repoInfo.org}/${repoName} tokenizer...`);

    // 下载 tokenizer.json
    const tokenizerDest = path.join(destDir, "tokenizer.json");
    await this.downloadFile(urls.tokenizer, tokenizerDest);
    logger.debug('TokenCounter', 'tokenizer.json 已下载');
    ...
    try {
      ...
      logger.debug('TokenCounter', 'tokenizer_config.json 已下载');
    } catch {
      logger.debug('TokenCounter', 'tokenizer_config.json 不存在，跳过');
    }
  }
  ...
  private async fallbackToDefault(modelName: string): Promise<Tokenizer> {
    ...
    if (this.defaultTokenizer) {
      if (!this.fallbackWarningPrinted) {
        ...
        logger.warn('TokenCounter', `${modelName} tokenizer 加载失败，降级使用默认 tokenizer`);
      }
      this.warnedModels.add(modelName);
  ...
  private async loadDefaultTokenizer(modelName: string): Promise<void> {
    ...
    try {
      ...
      if (!this.fallbackWarningPrinted) {
        this.fallbackWarningPrinted = true;
        logger.warn('TokenCounter', `${modelName} tokenizer 加载失败，降级使用 ${defaultRepo.org}/${defaultRepo.repo}`);
      }
    } catch (loadError) {
  ...
  private inferHfRepo(modelName: string): { org: string; repo: string; variant?: string } {
    ...
    if (!this.warnedModels.has(modelName)) {
      this.warnedModels.add(modelName);
      logger.warn('TokenCounter', `未知模型 "${modelName}", 使用默认 tokenizer`);
    }
  ...
  clearCache(): void {
    ...
    logger.debug('TokenCounter', '缓存已清除');
  }
}
...
export async function preloadTokenizer(modelName?: string): Promise<void> {
  ...
  try {
    ...
    logger.debug('TokenCounter', `预加载完成 (${model})`);
  } catch (error) {
    logger.error('TokenCounter', `预加载失败: ${error instanceof Error ? error.message : error}`);
  }
}
