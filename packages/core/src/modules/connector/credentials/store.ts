...
export class CredentialStore {
  ...
  constructor(options?: CredentialStoreOptions) {
    ...
    if (!rawKey) {
      ...
      logger.warn(
        'CredentialStore',
        'WARNING: Running in plaintext mode (no encryption key). ' +
  ...
  async initialize(): Promise<void> {
    ...
    if (fs.existsSync(this.storagePath)) {
      try {
        ...
        logger.debug('CredentialStore', `Loaded ${this.credentials.size} credentials`)
      } catch (error) {
        logger.error('CredentialStore', 'Failed to load credentials:', error)
      }
    }
  ...
  async set(connectorId: string, data: CredentialData): Promise<string> {
    ...
    logger.debug('CredentialStore', `Credential stored: ${connectorId}`)
  ...
  async get(connectorId: string): Promise<CredentialData | null> {
    ...
    if (credential) {
      try {
        ...
        logger.error('CredentialStore', `Decryption failed: ${connectorId}`, error)
        return null
      }
    }
  ...
  async delete(connectorId: string): Promise<boolean> {
    ...
    if (existed) {
      await this.persist()
      logger.debug('CredentialStore', `Credential deleted: ${connectorId}`)
    }
  ...
  async clear(): Promise<void> {
    ...
    await this.persist()
    logger.debug('CredentialStore', 'All credentials cleared')
  }
