...
import { getDatabase, type SqliteDatabase, type SqliteDatabaseConstructor } from '../../../services/datastore/sqlite/native-loader'
...
class PostgreSQLPool implements DatabasePool {
  ...
  constructor(config: DatabaseConnectionConfig) {
    ...
    // 实际部署时确保 pg 已安装
    logger.debug('PostgreSQLPool', `Configuration ready: ${config.postgresql?.host}`)
  }
  ...
  async connect(config: DatabaseConnectionConfig): Promise<void> {
    ...
    try {
      ...
      this.isConnected = true
      logger.debug('PostgreSQLPool', `Connected: ${this.connectionId}`)
    } catch (error) {
      // pg 库未安装
      logger.error('PostgreSQLPool', 'pg library not installed or connection failed:', error)
...
class MySQLPool implements DatabasePool {
  ...
  constructor(config: DatabaseConnectionConfig) {
    this.connectionId = config.connection_id
    logger.debug('MySQLPool', `Configuration ready: ${config.mysql?.host}`)
  }
  ...
  async connect(config: DatabaseConnectionConfig): Promise<void> {
    ...
    try {
      ...
      this.isConnected = true
      logger.debug('MySQLPool', `Connected: ${this.connectionId}`)
    } catch (error) {
      logger.error('MySQLPool', 'mysql2 library not installed or connection failed:', error)
...
export class DatabasePoolManager {
  ...
  registerConfig(config: DatabaseConnectionConfig): void {
    ...
    logger.debug('DatabasePoolManager', `Config registered: ${config.connection_id} ${config.type}`)
  }
  ...
  async closeAll(): Promise<void> {
    ...
    for (const [id, pool] of poolEntries) {
      await pool.close()
      logger.debug('DatabasePoolManager', `Pool closed: ${id}`)
    }
    this.pools.clear()
  }
