...
export class SQLiteInboundInbox implements InboundInbox {
  ...
  private dispatch(event: InboundEvent): void {
    Promise.resolve()
      ...
      .catch((error) => {
        logger.error('SQLiteInboundInbox', `Processing failed: ${event.id}`, error)
      })
  }
  ...
  private startHeartbeat(eventId: string): () => void {
    ...
    const timer = setInterval(() => {
      ...
      try {
        ...
        logger.error('SQLiteInboundInbox', `Failed to renew event lock: ${eventId}`, error)
      }
    ...
    try {
      ...
      for (const row of rows) {
        try {
          ...
          logger.error('SQLiteInboundInbox', 'Failed to dispatch pending event:', error)
        }
      }
    } finally {
