...
${conversationText}`;
    ...
    for (const memory of savableMemories.slice(0, 5)) {
      ...
      try {
        ...
        logger.error(
          'MemoryExtractor',
          `Failed to process memory "${memory.name}" (${memory.action}): ${err}`,
        );
      }
    }
    ...
    logger.error('MemoryExtraction', `Error: ${err}`);
    return { memories: [], count: 0 };
  }
}
...
export async function extractMemoriesInBackground(
  ...
  setImmediate(async () => {
    try {
      ...
      logger.error('MemoryExtraction', `Background extraction failed: ${err}`);
    }
  });
}
