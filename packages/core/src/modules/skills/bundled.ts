...
// ============================================================
//
// 内联在代码中，不依赖文件系统路径（避免打包环境下路径失效问题）。
// 优先级最低：user/project 目录下同名 skill 会覆盖此处定义。
//
// 如需添加新的内置 skill：
//   1. 在此数组追加一个对象
//   2. sourcePath 固定格式：'builtin:<name>'
//   3. body 包含完整的执行指令
// ============================================================

import type { Skill } from './types';

const CREATE_SKILL_BODY = `You are helping the user create a new Skill. A Skill is a reusable workflow defined in a SKILL.md file that can be invoked with \`/<name>\` in any conversation.
...
export const BUNDLED_SKILLS: Skill[] = [
  {
    name: 'create-skill',
    description: 'Create a new skill — generates a SKILL.md and writes it to the user skill directory',
    whenToUse: 'When the user wants to create a new skill, package a workflow as a reusable command, or types /create-skill',
    allowedTools: ['bash', 'write_file', 'ask_user_question'],
    effort: 'medium',
    context: 'fork',
    paths: [],
