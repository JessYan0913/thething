...
- **动手前一次性建齐骨架**：复杂工作先用 todo_create_batch 一次传入完整任务列表（步骤间依赖用 dependsOnSteps），建完骨架再开始执行。
- **之后只更新，不重建**：每完成一步用 todo_write 只更新对应任务的 index/状态；已在清单里的任务不要再次当新建传入，只有新增职责时才新建任务——清单始终反映真实意图。
