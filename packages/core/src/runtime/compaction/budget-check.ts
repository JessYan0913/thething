...
export async function checkInitialBudget(
  ...
  console.log(`[Budget] Model: ${modelName}, Limit: ${initialEstimation.modelLimit}`);
  console.log(`[Budget] Initial: ${initialEstimation.totalTokens} tokens (${initialEstimation.utilizationPercent.toFixed(1)}%)`);

  if (!initialEstimation.exceedsLimit) {
    return { passed: true, estimation: initialEstimation, actions: ['Budget check passed'] };
  }

  console.warn(`[Budget] ⚠️ Exceeds limit: ${initialEstimation.totalTokens} > ${initialEstimation.modelLimit}`);
  ...
  if (currentEstimation.messagesTokens > currentEstimation.modelLimit * 0.2) {
    ...
    if (lifecycleResult.tokensFreed > 0) {
      ...
      console.log(`[Budget] After Layer 2: ${currentEstimation.totalTokens} tokens`);
  ...
  if (currentEstimation.toolsTokens > currentEstimation.modelLimit * TOOL_BUDGET_RATIO) {
    ...
    if (removed > 0) {
      ...
      currentEstimation = await estimateFullRequest(currentMessages, instructions, currentTools, modelName);
      console.log(`[Budget] After tool filter: ${currentEstimation.totalTokens} tokens`);
  ...
  if (context?.conversationId && context?.model && currentEstimation.messagesTokens > currentEstimation.modelLimit * 0.3) {
    try {
      ...
      if (windowResult.executed) {
        ...
        currentEstimation = await estimateFullRequest(currentMessages, instructions, currentTools, modelName);
        console.log(`[Budget] After Layer 3: ${currentEstimation.totalTokens} tokens`);
      ...
      console.warn('[Budget] Layer 3 failed:', err);
    }
  }
  ...
  console.log(`[Budget] Final: ${finalEstimation.totalTokens} tokens (${finalEstimation.utilizationPercent.toFixed(1)}%) - ${finalEstimation.exceedsLimit ? '❌ EXCEEDS' : '✅ OK'}`);
